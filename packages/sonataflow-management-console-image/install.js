...
execSync(
  `${activateCmd} && \
  python3 ${sonataflowImageCommonDir}/resources/scripts/versions_manager.py --bump-to ${buildEnv.env.sonataflowManagementConsoleImage.version} --source-folder ./resources`,
  { stdio: "inherit" }
);
...
const imageUrl = `${buildEnv.env.sonataflowManagementConsoleImage.registry}/${buildEnv.env.sonataflowManagementConsoleImage.account}/${buildEnv.env.sonataflowManagementConsoleImage.name}`;

// Replace the whole string between quotes ("") with the image name
imageYaml = imageYaml.replace(/(?<=")(.*sonataflow-management-console.*)(?=")/gm, imageUrl);

// Write file and then rename it to match the image name
fs.writeFileSync(originalYamlPath, imageYaml);
fs.renameSync(
  originalYamlPath,
  path.join(resourcesPath, `${buildEnv.env.sonataflowManagementConsoleImage.name}-image.yaml`)
);
