...
Feature: Serverless Workflow management-console images
  ...
  Scenario: Verify if container starts in management-console by default
  ...
  Scenario: Verify if container starts in management-console with service discovery enabled
    When container is started with env
      | variable                    | value |
      | QUARKUS_DEVSERVICES_ENABLED | false |
    Then check that page is served
